...
#define PING_COUNT 10 //count of pings per request
#define DELAY 60000 // This is 1 minute , Time to wait after completing a full ping request (doing PING_COUNT pings) and before the next one
long int Delay_ping;
...
IPAddress awakened_ip(000,000,000,000); 
IPAddress     ping_ip(8,8,4,4);

/**
 * The targets MAC address to send the packet to
 */

byte awakened_mac[] = {0x00,0x00,0x00,0x00,0x0,0x00 };
...
static void ICACHE_FLASH_ATTR ping_sent (void* arg, void *pdata)
{
  ...
  package_loss=float(pingrsp->timeout_count)/pingrsp->total_count*100;
  //Serial.println("% loss)");

  // Ping request complete, rearm the timer so that a new request is sent in DELAY seconds
  os_timer_arm(&ping_timer, Delay_ping, 1);
}
...
void ip_maker(byte x , String IP_string) {

 char cstringToParse[17];
 uint8_t ip[4];

strcpy(cstringToParse, IP_string.c_str()); 
...
void mac_maker(String IP_string) {

 char cstringToParse[17];
 uint8_t ip[6];
 
parseBytes(IP_string.c_str(), '-',  awakened_mac, 6, 16);
...
void setup() {
 ...
 // Odczyt zapamiętanych danych dl WoL i Ping
 ip_maker(0,read_supla_awakened_ip().c_str());
 Serial.print("IP PC WoL :");
 Serial.println(awakened_ip);
  
 mac_maker(read_supla_awakened_mac().c_str()); 
 Serial.print("MAC PC WoL :");
 for (byte a=0; a<6; a++) {
      Serial.print(awakened_mac[a],HEX);
      if (a < 5) Serial.print("-"); 
 }
 
 ip_maker(1,read_supla_ping_ip().c_str());
 Serial.println();
 ...
 Serial.println(ping_ip);
  ...
  String Delay_ping_s = read_supla_ping_time().c_str();
 
  Delay_ping = Delay_ping_s.toInt() *DELAY; // [min]
  if (Delay_ping < DELAY) Delay_ping = DELAY; //not less than a minute
  Serial.print("TIMING PING :");Serial.print(Delay_ping/DELAY);Serial.println(" minutes :=>"); 
  os_timer_arm(&ping_timer,Delay_ping, 1);
...
void supla_DigitalWrite(int channelNumber, uint8_t pin, uint8_t val) {
  ...
  /*Serial.print("Write(");
    ...
    Serial.print(val);
    Serial.println(")");*/
  if (pin < 100) digitalWrite(pin, val);
  else {
     if (val==1) {
      digitalWrite(LED_CONFIG_PIN, LOW);   
      WakeOnLan::sendWOL(awakened_ip, UDP, awakened_mac, sizeof awakened_mac);
...
void createWebServer() {
  ...
  httpServer.on("/save_awankened", []() {
    if (Modul_tryb_konfiguracji == 0) {
      if (!httpServer.authenticate(www_username, www_password))
        return httpServer.requestAuthentication();
    }

    save_supla_awakened_ip(httpServer.arg("awakened_ip_pc"));
    save_supla_awakened_mac(httpServer.arg("awakened_mac_pc"));
    
    httpServer.send(200, "text/html", supla_webpage_wakeup(1));
  });

//*************************************************************
httpServer.on("/save_ping", []() {
    if (Modul_tryb_konfiguracji == 0) {
      if (!httpServer.authenticate(www_username, www_password))
        return httpServer.requestAuthentication();
    }

    save_supla_ping_ip(httpServer.arg("ping_ip_pc"));
    save_supla_ping_time(httpServer.arg("ping_time_pc"));
    
    httpServer.send(200, "text/html", supla_webpage_ping(1));
  });
...
//*************************************************************
  ...
  httpServer.on("/WakeUp", []() {
    if (Modul_tryb_konfiguracji == 0) {
      if (!httpServer.authenticate(www_username, www_password))
        return httpServer.requestAuthentication();
    }
    httpServer.send(200, "text/html", supla_webpage_wakeup(0));
  });
  httpServer.on("/Ping", []() {
    if (Modul_tryb_konfiguracji == 0) {
      if (!httpServer.authenticate(www_username, www_password))
    ...
    httpServer.send(200, "text/html", supla_webpage_ping(0));
  });
  ...
  *humidity = package_loss;
  ...
  if ( isnan(*temp) || isnan(*humidity) ) {
    ...
    *humidity = -1;
    ...
    Serial.print("Time delay- "); Serial.print(Delay_time);Serial.print("   lost packets -");Serial.println(package_loss);
}
